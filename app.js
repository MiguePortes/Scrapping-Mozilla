  ...
  try {
    for (let pageNum = 1; pageNum <= lastPage; pageNum++) {
      ...
      const previews = await page.evaluate(() => {
        return Array.from(document.querySelectorAll('ul.article-list li')).map(article => {
      ...
      for (const preview of previews) {
        try {
          await page.goto(preview.url, { waitUntil: 'networkidle2', timeout: 0 });
          await page.waitForSelector('h1', { timeout: 30000 });

          const details = await page.evaluate(() => {
            const author = document.querySelector('.post-meta a')?.innerText || 'Autor no encontrado';
            const date = document.querySelector('abbr.published')?.getAttribute('title') || 'Fecha no encontrada';
            const firstParagraph = Array.from(document.querySelectorAll('.post-content p'))
              .find(p => p.offsetParent !== null && p.innerText.trim().length > 30)?.innerText.trim() || 'Párrafo no encontrado';
            const avatar = document.querySelector('img.avatar')?.getAttribute('src') || 'Imagen no encontrada';
    ...
    // JSON
    fs.writeFileSync('articulos-mozilla.json', JSON.stringify(allArticles, null, 2), 'utf-8');
